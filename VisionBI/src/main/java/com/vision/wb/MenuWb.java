...
public class MenuWb extends AbstractDynaWorkerBean<MenuVb>{
	...
	public ArrayList getPageLoadValues(){
		...
		try{
			...
			arrListLocal.add(collTemp);
			collTemp = numSubTabDao.findActiveNumSubTabsByNumTab(176);
			arrListLocal.add(collTemp);
			collTemp = commonDao.findVerificationRequiredAndStaticDelete("VISION_MENU");
			arrListLocal.add(collTemp);
	...
	protected void setVerifReqDeleteType(MenuVb vObject) {
		ArrayList<CommonVb> lCommVbList =(ArrayList<CommonVb>) commonDao.findVerificationRequiredAndStaticDelete("VISION_MENU");
	...
	public ExceptionCode getQueryResults(MenuVb vObject){
		...
		if(collTemp.size() == 0){
			...
			for(MenuVb vObj : collTemp) {
				List<MenuVb> subMenulst = menuDao.getQueryResultsSubMenu(vObj, intStatus);
