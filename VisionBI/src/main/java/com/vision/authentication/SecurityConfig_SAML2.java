...
public class SecurityConfig_SAML2 {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.cors().and().csrf().disable()
				.authorizeHttpRequests(authorize -> authorize.antMatchers(HttpMethod.OPTIONS, "/**").permitAll() // Allow
																													// access
																													// to
																													// any
																													// request
																													...
																													// without
																													// authentication
						.antMatchers(allowedURLs.split(",")).permitAll() // Allow access to /refreshToken without
																	// authentication
