...
public class MenuDao extends AbstractDao<MenuVb> {
	...
	public List<MenuVb> getQueryPopupResults(MenuVb dObj){
		...
		StringBuffer strBufApprove = new StringBuffer("Select Distinct TAppr.MENU_GROUP," +
		    " (Select Num_SubTab_description from Num_sub_tab where Num_tab = 176 and Num_sub_tab = Tappr.Menu_Group ) Menu_Group_Name "+
			" From VISION_MENU TAppr ");

		String strWhereNotExists = new String(" Not Exists (Select 'X' From VISION_MENU_PEND TPend Where " + 
		...
		StringBuffer strBufPending = new StringBuffer("Select Distinct TPend.MENU_GROUP," + 
			" (Select Num_SubTab_description from Num_sub_tab where Num_tab = 176 and Num_sub_tab = TPend.Menu_Group ) Menu_Group_Name "+
			" From VISION_MENU_PEND TPend ");
	...
	public List<MenuVb> getQueryResults(MenuVb dObj, int intStatus){
		Vector<Object> params = new Vector<Object>();
		setServiceDefaults();
		StringBuffer strBufApprove = new StringBuffer("Select TAppr.MENU_PROGRAM,TAppr.MENU_NAME, " + 
			...
			" 'DD-MON-YYYY HH24:MI:SS') DATE_LAST_MODIFIED,To_Char(TAppr.DATE_CREATION, 'DD-MON-YYYY HH24:MI:SS') DATE_CREATION From VISION_MENU TAppr "
		...
		StringBuffer strBufPending = new StringBuffer("Select TPend.MENU_PROGRAM,TPend.MENU_NAME, " + 
			...
			" 'DD-MON-YYYY HH24:MI:SS') DATE_LAST_MODIFIED,To_Char(TPend.DATE_CREATION, 'DD-MON-YYYY HH24:MI:SS') DATE_CREATION From VISION_MENU_PEND TPend "
	...
	public List<MenuVb> getQueryResultsSubMenu(MenuVb dObj, int intStatus){
		...
		StringBuffer strBufApprove = new StringBuffer("Select TAppr.MENU_PROGRAM,TAppr.MENU_NAME, " + 
			...
			" (SELECT MIN(USER_NAME) FROM VISION_USERS WHERE VISION_ID = NVL(TAPPR.VERIFIER,0) ) VERIFIER_NAME,"+
			" TAppr.INTERNAL_STATUS,To_Char(TAppr.DATE_LAST_MODIFIED, " + 
			" 'DD-MON-YYYY HH24:MI:SS') DATE_LAST_MODIFIED,To_Char(TAppr.DATE_CREATION, 'DD-MON-YYYY HH24:MI:SS') DATE_CREATION, "
			+ " TAppr.APPLICATION_ACCESS_AT,TAppr.APPLICATION_ACCESS From VISION_MENU TAppr "
			+ "where TAppr.Menu_Sequence <> TAppr.Parent_Sequence and TAppr.Separator <> 'Y' and (TAppr.APPLICATION_ACCESS LIKE '%"+productName+"%' ) " );
		...
		StringBuffer strBufPending = new StringBuffer("Select TPend.MENU_PROGRAM,TPend.MENU_NAME, " + 
			...
			" (SELECT MIN(USER_NAME) FROM VISION_USERS WHERE VISION_ID = NVL(TPend.VERIFIER,0) ) VERIFIER_NAME,"+
			" TPend.INTERNAL_STATUS,To_Char(TPend.DATE_LAST_MODIFIED, " + 
			" 'DD-MON-YYYY HH24:MI:SS') DATE_LAST_MODIFIED,To_Char(TPend.DATE_CREATION, 'DD-MON-YYYY HH24:MI:SS') DATE_CREATION, "
			+ " TPend.APPLICATION_ACCESS_AT,TPend.APPLICATION_ACCESS From VISION_MENU_PEND TPend "
			+ "where TPend.Menu_Sequence <> TPend.Parent_Sequence and TPend.Separator <> 'Y' and (TPend.APPLICATION_ACCESS LIKE '%"+productName+"%' ) ");
	...
	public List<MenuVb> getQueryResultsForReview(MenuVb dObj, int intStatus){
		...
		StringBuffer strQueryAppr = new StringBuffer("Select TAppr.MENU_PROGRAM,TAppr.MENU_NAME, " + 
			"TAppr.MENU_SEQUENCE,TAppr.PARENT_SEQUENCE,TAppr.SEPARATOR,TAppr.MENU_GROUP_NT,TAppr.MENU_GROUP, " + 
			"TAppr.MENU_STATUS_NT,TAppr.MENU_STATUS,TAppr.MENU_NODE_VISIBILITY ,TAppr.RECORD_INDICATOR_NT,TAppr.RECORD_INDICATOR, " + 
			"TAppr.MAKER,TAppr.VERIFIER,TAppr.INTERNAL_STATUS,To_Char(TAppr.DATE_LAST_MODIFIED, 'DD-MON-YYYY HH24:MI:SS') DATE_LAST_MODIFIED, " +
			"To_Char(TAppr.DATE_CREATION, 'DD-MON-YYYY HH24:MI:SS') DATE_CREATION,TAppr.APPLICATION_ACCESS_AT,TAppr.APPLICATION_ACCESS " +
			" From VISION_MENU TAppr " + 
			"Where TAppr.MENU_PROGRAM = ?  And TAppr.MENU_GROUP = ?  and TAppr.APPLICATION_ACCESS LIKE '%"+productName+"%' ");

		StringBuffer strQueryPend = new StringBuffer("Select TPend.MENU_PROGRAM,TPend.MENU_NAME, " + 
			"TPend.MENU_SEQUENCE,TPend.PARENT_SEQUENCE,TPend.SEPARATOR,TPend.MENU_GROUP_NT,TPend.MENU_GROUP, " + 
			"TPend.MENU_STATUS_NT,TPend.MENU_STATUS,TPend.MENU_NODE_VISIBILITY ,TPend.RECORD_INDICATOR_NT,TPend.RECORD_INDICATOR, " + 
			"TPend.MAKER,TPend.VERIFIER,TPend.INTERNAL_STATUS,To_Char(TPend.DATE_LAST_MODIFIED, 'DD-MON-YYYY HH24:MI:SS') DATE_LAST_MODIFIED," +
			"To_Char(TPend.DATE_CREATION, 'DD-MON-YYYY HH24:MI:SS') DATE_CREATION,TPend.APPLICATION_ACCESS_AT,TPend.APPLICATION_ACCESS " +
			" From VISION_MENU_PEND TPend " + 
			" Where TPend.MENU_PROGRAM = ?  And TPend.MENU_GROUP = ? and TPend.APPLICATION_ACCESS LIKE '%"+productName+"%' ");
	...
	protected void setServiceDefaults(){
		serviceName = "Menu";
		serviceDesc = "Menu";
		tableName = "VISION_MENU";
		childTableName = "VISION_MENU";
		intCurrentUserId = CustomContextHolder.getContext().getVisionId();
	}

	@Override
	protected int getStatus(MenuVb records){return records.getMenuStatus();}

	@Override
	protected void setStatus(MenuVb vObject,int status){vObject.setMenuStatus(status);}

	@Override
	protected int doInsertionAppr(MenuVb vObject){
		String query = "Insert Into VISION_MENU( " + 
		...
		Object args[] = {vObject.getMenuProgram(), vObject.getMenuName(), vObject.getMenuSequence(),
			 ...
			 vObject.getMenuStatusNt(), vObject.getMenuStatus(),vObject.getMenuNodeAvailable(), vObject.getRecordIndicatorNt(),
			 vObject.getRecordIndicator(), vObject.getMaker(), vObject.getVerifier(), vObject.getInternalStatus(),
			 vObject.getApplicationAccessAt(),productName};
	...
	protected int doInsertionPend(MenuVb vObject){
		String query = "Insert Into VISION_MENU_PEND( " + 
		"MENU_PROGRAM, MENU_NAME, MENU_SEQUENCE, PARENT_SEQUENCE, SEPARATOR, MENU_GROUP_NT, " + 
			" MENU_GROUP, MENU_STATUS_NT, MENU_STATUS, MENU_NODE_VISIBILITY, RECORD_INDICATOR_NT, RECORD_INDICATOR, " + 
			" MAKER, VERIFIER, INTERNAL_STATUS, DATE_LAST_MODIFIED, DATE_CREATION,APPLICATION_ACCESS_AT,APPLICATION_ACCESS) " + 
			" Values (?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?, SysDate, SysDate, ?, ?)";
	...
	protected int doInsertionPendWithDc(MenuVb vObject){
		...
		Object args[] = {vObject.getMenuProgram(), vObject.getMenuName(), vObject.getMenuSequence(), 
			 ...
			 vObject.getMenuStatusNt(), vObject.getMenuStatus(),vObject.getMenuNodeAvailable(), vObject.getRecordIndicatorNt(), 
			 vObject.getRecordIndicator(), vObject.getMaker(), vObject.getVerifier(), vObject.getInternalStatus(), 
			 vObject.getDateCreation(),vObject.getApplicationAccessAt(),productName};
	...
	protected int doUpdateAppr(MenuVb vObject){
		String query = "Update VISION_MENU Set " + 
		...
		Object args[] = {vObject.getMenuName(), vObject.getMenuSequence(), vObject.getParentSequence(),
		 vObject.getSeparator(), vObject.getMenuGroupNt(), vObject.getMenuStatusNt(), vObject.getMenuStatus(), vObject.getMenuNodeAvailable(),
		 vObject.getRecordIndicatorNt(), vObject.getRecordIndicator(), vObject.getMaker(),
		 vObject.getVerifier(), vObject.getInternalStatus(), 
		 vObject.getMenuProgram(), 
	...
	protected int doUpdatePend(MenuVb vObject){
		String query = "Update VISION_MENU_PEND Set " + 
		"MENU_NAME = ?, MENU_SEQUENCE = ?, PARENT_SEQUENCE = ?, SEPARATOR = ?, MENU_GROUP_NT = ?," +
		"MENU_STATUS_NT = ?, MENU_STATUS = ?, MENU_NODE_VISIBILITY = ?, RECORD_INDICATOR_NT = ?, RECORD_INDICATOR = ?," +
		"MAKER = ?, VERIFIER = ?, INTERNAL_STATUS = ?, DATE_LAST_MODIFIED = SysDate " + 
		"Where MENU_PROGRAM = ? " +
	...
	protected int doDeleteAppr(MenuVb vObject){
		String query = "Delete From VISION_MENU Where " + 
		"MENU_PROGRAM = ? And " + 
		"MENU_GROUP = ? " ;
	...
	protected int deletePendingRecord(MenuVb vObject){
		String query = "Delete From VISION_MENU_PEND Where " + 
		"MENU_PROGRAM = ? And " + 
		"MENU_GROUP = ? " ;
