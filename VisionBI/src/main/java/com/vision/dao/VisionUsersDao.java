...
public class VisionUsersDao extends AbstractDao<VisionUsersVb> implements ApplicationContextAware {
	...
	public List<VisionUsersVb> getQueryPopupResults(VisionUsersVb dObj) {
		...
		StringBuffer strBufApprove = new StringBuffer("Select TAppr.VISION_ID," + "TAppr.USER_NAME, TAppr.USER_LOGIN_ID, TAppr.USER_EMAIL_ID,"
				...
				+ "TAppr.COUNTRY, TAppr.LE_BOOK," + "TAppr.REGION_PROVINCE, TAppr.BUSINESS_GROUP, TAppr.PRODUCT_SUPER_GROUP,"
		...
		StringBuffer strBufPending = new StringBuffer("Select TPend.VISION_ID," + "TPend.USER_NAME, TPend.USER_LOGIN_ID, TPend.USER_EMAIL_ID,"
				...
				+ "TPend.COUNTRY, TPend.LE_BOOK," + "TPend.REGION_PROVINCE, TPend.BUSINESS_GROUP, TPend.PRODUCT_SUPER_GROUP,"
		...
		try {
			...
			if (ValidationUtil.isValid(dObj.getLastActivityDate())) {
				...
				if (endDate.trim().indexOf(" ") > -1) {
					...
					params.addElement(endDate.trim() + " 00:00:00");
					params.addElement(endDate.trim() + " 23:59:59");
					CommonUtils.addToQuery("TAppr.LAST_ACTIVITY_DATE BETWEEN To_Date(?, 'DD-MM-YYYY HH24:MI:SS') AND To_Date(?, 'DD-MM-YYYY HH24:MI:SS') ",
							strBufApprove);
					CommonUtils.addToQuery("TPend.LAST_ACTIVITY_DATE BETWEEN To_Date(?, 'DD-MM-YYYY HH24:MI:SS') AND To_Date(?, 'DD-MM-YYYY HH24:MI:SS') ",
							strBufPending);
				}
			}
	...
	public boolean updateActivityDateByUserLoginId(VisionUsersVb visionUsersVb) {
		...
		int count = getJdbcTemplate().update(
				"Update VISION_USERS SET LAST_ACTIVITY_DATE = SYSDATE, " + "LAST_UNSUCCESSFUL_LOGIN_DATE = null, UNSUCCESSFUL_LOGIN_ATTEMPTS = null  WHERE VISION_ID = ?",
				params);
		return count == 1;
	}
	...
	public List<VisionUsersVb> getActiveUserByUserLoginId(VisionUsersVb dObj) {
		...
		StringBuffer strQueryAppr = new StringBuffer("Select TAppr.VISION_ID," + "TAppr.USER_NAME, TAppr.USER_LOGIN_ID, TAppr.USER_EMAIL_ID,"
				+ "To_Char(TAppr.LAST_ACTIVITY_DATE, 'DD-MM-YYYY HH24:MI:SS') LAST_ACTIVITY_DATE, TAppr.USER_GROUP_AT,"
				+ "TAppr.USER_GROUP, TAppr.USER_PROFILE_AT, TAppr.USER_PROFILE,"
				+ "TAppr.UPDATE_RESTRICTION, TAppr.LEGAL_VEHICLE, (Select LV_DESCRIPTION From LEGAL_VEHICLES Where LEGAL_VEHICLE = TAppr.LEGAL_VEHICLE) AS LegalVehicleDesc,"
				+ "TAppr.COUNTRY, TAppr.LE_BOOK," + "TAppr.REGION_PROVINCE, TAppr.BUSINESS_GROUP, TAppr.PRODUCT_SUPER_GROUP,"
	...
	public List<VisionUsersVb> getQueryResults(VisionUsersVb dObj, int intStatus) {
		...
		String strQueryAppr = new String("Select TAppr.VISION_ID," + "TAppr.USER_NAME, TAppr.USER_LOGIN_ID, TAppr.USER_EMAIL_ID,"
				+ "To_Char(TAppr.LAST_ACTIVITY_DATE, 'DD-MM-YYYY HH24:MI:SS') LAST_ACTIVITY_DATE, TAppr.USER_GROUP_AT,"
				+ "TAppr.USER_GROUP, TAppr.USER_PROFILE_AT, TAppr.USER_PROFILE,"
				+ "TAppr.UPDATE_RESTRICTION, TAppr.LEGAL_VEHICLE, (Select LV_DESCRIPTION From LEGAL_VEHICLES Where LEGAL_VEHICLE = TAppr.LEGAL_VEHICLE) AS LegalVehicleDesc,"
				+ "TAppr.COUNTRY, TAppr.LE_BOOK," + "TAppr.REGION_PROVINCE, TAppr.BUSINESS_GROUP, TAppr.PRODUCT_SUPER_GROUP,"
				...
				+ " From VISION_USERS TAppr " + "Where TAppr.VISION_ID = ? ");
		String strQueryPend = new String("Select TPend.VISION_ID," + "TPend.USER_NAME, TPend.USER_LOGIN_ID, TPend.USER_EMAIL_ID,"
				+ "To_Char(TPend.LAST_ACTIVITY_DATE, 'DD-MM-YYYY HH24:MI:SS') LAST_ACTIVITY_DATE, TPend.USER_GROUP_AT,"
				+ "TPend.USER_GROUP, TPend.USER_PROFILE_AT, TPend.USER_PROFILE,"
				+ "TPend.UPDATE_RESTRICTION, TPend.LEGAL_VEHICLE, (Select LV_DESCRIPTION From LEGAL_VEHICLES Where LEGAL_VEHICLE = TPend.LEGAL_VEHICLE) AS LegalVehicleDesc,"
				+ "TPend.COUNTRY, TPend.LE_BOOK," + "TPend.REGION_PROVINCE, TPend.BUSINESS_GROUP, TPend.PRODUCT_SUPER_GROUP,"
				...
				+ " From VISION_USERS_PEND TPend " + "Where TPend.VISION_ID = ? ");
	...
	protected int doInsertionAppr(VisionUsersVb vObject) {
		...
		Object[] args = { vObject.getVisionId(), vObject.getUserName(), vObject.getUserLoginId(), vObject.getUserEmailId(), vObject.getLastActivityDate(),
				vObject.getUserGroupAt(), vObject.getUserGroup(), vObject.getUserProfileAt(), vObject.getUserProfile(), vObject.getUpdateRestriction(),
				vObject.getLegalVehicle(), vObject.getCountry(), vObject.getLeBook(), vObject.getRegionProvince(), vObject.getBusinessGroup(),
				vObject.getProductSuperGroup(), vObject.getOucAttribute(), vObject.getSbuCodeAt(), vObject.getSbuCode(), vObject.getProductAttribute(),
				vObject.getAccountOfficer(), vObject.getGcidAccess(), vObject.getUserStatusNt(), vObject.getUserStatus(), vObject.getUserStatusDate(), vObject.getMaker(),
				vObject.getVerifier(), vObject.getInternalStatus(), vObject.getRecordIndicatorNt(), vObject.getRecordIndicator(), vObject.getFileNmae(),
				vObject.getEnableWidgets() };
	...
	protected int doInsertionPendWithDc(VisionUsersVb vObject) {
		...
		Object[] args = { vObject.getVisionId(), vObject.getUserName(), vObject.getUserLoginId(), vObject.getUserEmailId(), vObject.getLastActivityDate(),
				...
				vObject.getProductSuperGroup(), vObject.getOucAttribute(), vObject.getSbuCodeAt(), vObject.getSbuCode(), vObject.getProductAttribute(),
				vObject.getAccountOfficer(), vObject.getGcidAccess(), vObject.getUserStatusNt(), vObject.getUserStatus(), vObject.getUserStatusDate(), vObject.getMaker(),
				vObject.getVerifier(), vObject.getInternalStatus(), vObject.getRecordIndicatorNt(), vObject.getRecordIndicator(), vObject.getDateCreation(),
	...
	protected int doUpdateAppr(VisionUsersVb vObject) {
		String query = "Update VISION_USERS Set USER_LOGIN_ID = ?, USER_NAME = ?, USER_EMAIL_ID = ?," + "LAST_ACTIVITY_DATE = To_Date(?, 'DD-MM-YYYY HH24:MI:SS'),"
		...
		Object[] args = { vObject.getUserLoginId(), vObject.getUserName(), vObject.getUserEmailId(), vObject.getLastActivityDate(), vObject.getLegalVehicle(),
				...
				vObject.getAccountOfficer(), vObject.getUserStatusNt(), vObject.getUserStatus(), vObject.getUserStatusDate(), vObject.getMaker(), vObject.getVerifier(),
				vObject.getInternalStatus(), vObject.getRecordIndicatorNt(), vObject.getRecordIndicator(), vObject.getFileNmae(), vObject.getEnableWidgets(),
				vObject.getVisionId() };
		return getJdbcTemplate().update(query, args);
	}
	...
	protected int doUpdatePend(VisionUsersVb vObject) {
		String query = "Update VISION_USERS_PEND Set USER_LOGIN_ID = ?, USER_NAME = ?, USER_EMAIL_ID = ?," + "LAST_ACTIVITY_DATE = To_Date(?, 'DD-MM-YYYY HH24:MI:SS'),"
				+ "LEGAL_VEHICLE = ?,COUNTRY = ?, LE_BOOK = ?, REGION_PROVINCE = ?, BUSINESS_GROUP = ?, PRODUCT_SUPER_GROUP = ?, "
				+ "USER_GROUP_AT = ?, USER_GROUP = ?, USER_PROFILE_AT = ?, USER_PROFILE = ?, UPDATE_RESTRICTION = ?,"
				+ "OUC_ATTRIBUTE = ?, GCID_ACCESS = ?, SBU_CODE_AT = ?, SBU_CODE = ?, PRODUCT_ATTRIBUTE = ?, ACCOUNT_OFFICER = ?,"
				+ "USER_STATUS_NT = ?, USER_STATUS = ?, USER_STATUS_DATE = To_Date(?, 'DD-MM-YYYY HH24:MI:SS'),"
	...
	public String getCurrentPeriod() {
		return getJdbcTemplate().queryForObject("select to_char(to_date(CURRENT_YEAR_MONTH,'RRRRMM'),'RRRRMMDD') CURRENT_YEAR_MONTH from V_Curr_Year_Month",
				String.class);
	}
	...
	public List<VisionUsersVb> getPromptDataByUserLoginId(VisionUsersVb dObj) {
		...
		StringBuffer strQueryAppr = new StringBuffer("SELECT NVL(T1.LEGAL_VEHICLE,T2.LEGAL_VEHICLE) LEGAL_VEHICLE, T1.COUNTRY,T1.LE_BOOK, "
				...
				+ " WHEN LEGAL_VEHICLE IS NOT NULL THEN (select  COUNTRY from LE_BOOK X1" + " WHERE   X1.LEB_STATUS=0 AND X1.RECORD_INDICATOR=0"
				...
				+ " WHEN LEGAL_VEHICLE IS NOT NULL THEN (select LE_BOOK from LE_BOOK X1 " + " WHERE   X1.LEB_STATUS=0 AND X1.RECORD_INDICATOR=0 "
				+ " AND X1.LEGAL_VEHICLE= T2.LEGAL_VEHICLE) " + " ELSE '99' END) LE_BOOK , " + " LEGAL_VEHICLE LEGAL_VEHICLE, " + " OUC_ATTRIBUTE VISION_OUC "
				+ " from VISION_USERS T2 WHERE UPPER(USER_LOGIN_ID)=?) t1 , LE_BOOk T2 " + " WHERE T1.COUNTRY=T2.COUNTRY AND T1.LE_BOOK=T2.LE_BOOK");
		try {
			objParams[0] = dObj.getUserLoginId();
	...
	public VisionUsersVb callProcToPopulateForgotPasswordEmail(VisionUsersVb vObj, String resultForgotBy) {
		...
		strErrorDesc = "";
		try (Connection con = getConnection(); CallableStatement cs = con.prepareCall("{call PR_FORGOT_PWD_CHK_V1(?, ? , ?, ?)}");) {
	...
	public Map<String, List> doUpdateRestrictionToUserObject(VisionUsersVb vObject, Map<String, List> restrictionTree) {
		...
		for (Map.Entry<String, List> entry : restrictionTree.entrySet()) {
			...
			String restrictedValue = userObjectUpdate(vObject, entry.getKey(), getJdbcTemplate().query(sql, args, getMapperRestriction()));
	...
	public int insertUserLoginAudit(VisionUsersVb vObject) {
		...
		try {
			String query = "Insert Into DQ_USERS_LOGIN_AUDIT (USER_LOGIN_ID,VISION_ID,IP_ADDRESS,HOST_NAME,ACCESS_DATE,LOGIN_STATUS_AT,LOGIN_STATUS,COMMENTS,MAC_ADDRESS, NODE_NAME) "
					+ " Values (?, ?, ?, ?, SYSDATE, 1206, ?, ?, ?, ?)";
			Object[] args = { vObject.getUserLoginId(), vObject.getVisionId(), vObject.getIpAddress(), vObject.getRemoteHostName(), vObject.getLoginStatus(),
	...
	public List<VisionUsersVb> getUserInfoForLogin(VisionUsersVb dObj){
		...
		Object objParams[] = new Object[1];
		StringBuffer strQueryAppr = new StringBuffer("SELECT VISION_ID,USER_NAME,USER_LOGIN_ID,USER_EMAIL_ID,LAST_ACTIVITY_DATE,USER_GROUP,USER_PROFILE, "+
				...
				" APPLICATION_ACCESS, (SELECT App_Theme FROM PRD_APP_THEME S1 WHERE S1.VISION_ID=TAppr.VISION_ID and Application_id = '"+productName+"') APP_THEME,  "+
				...
				" From VISION_USERS_VW TAppr WHERE USER_STATUS = 0 "+
