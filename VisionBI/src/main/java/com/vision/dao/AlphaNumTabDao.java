...
public class AlphaNumTabDao extends AbstractDao<TabVb> {
	...
	public List<TabVb> getQueryPopupResults(TabVb tabVb){
		if("numTab".equalsIgnoreCase(tabVb.getRequestType())){
			...
			return getQueryPopupResultsNtNew(tabVb);
		}else{
			setServiceDefaultsAt();
			return getQueryPopupResultsAtNew(tabVb);
		}
	}
	...
	private List<TabVb> getQueryResultsAt(TabVb tabVb, int intStatus) {
		...
		String strQueryAppr = new String("Select ALPHA_TAB, ALPHA_TAB_DESCRIPTION, ALPHA_TAB_STATUS_NT, ALPHA_TAB_STATUS, MAKER, " +
				"RECORD_INDICATOR_NT, RECORD_INDICATOR, VERIFIER, "+dateFormat+"(DATE_LAST_MODIFIED, "+dateFormatStr+") DATE_LAST_MODIFIED, " +
				dateFormat+"(DATE_CREATION, "+dateFormatStr+") DATE_CREATION, INTERNAL_STATUS, READ_ONLY From ALPHA_TAB Where ALPHA_TAB = ? ");
		String strQueryPend = new String("Select ALPHA_TAB, ALPHA_TAB_DESCRIPTION, ALPHA_TAB_STATUS_NT, ALPHA_TAB_STATUS, MAKER, " +
				"RECORD_INDICATOR_NT, RECORD_INDICATOR, VERIFIER, "+dateFormat+"(DATE_LAST_MODIFIED, "+dateFormatStr+") DATE_LAST_MODIFIED, " +
				dateFormat+"(DATE_CREATION, "+dateFormatStr+") DATE_CREATION, INTERNAL_STATUS, READ_ONLY From ALPHA_TAB_PEND Where ALPHA_TAB = ? ");
	...
	private List<TabVb> getQueryResultsNt(TabVb tabVb,int intStatus) {
		...
		String strQueryAppr = new String("Select NUM_TAB, NUM_TAB_DESCRIPTION, NUM_TAB_STATUS_NT, NUM_TAB_STATUS, MAKER, " +
				"RECORD_INDICATOR_NT, RECORD_INDICATOR, VERIFIER, "+dateFormat+"(DATE_LAST_MODIFIED, "+dateFormatStr+") DATE_LAST_MODIFIED, " +
				dateFormat+"(DATE_CREATION, "+dateFormatStr+") DATE_CREATION, INTERNAL_STATUS, READ_ONLY From NUM_TAB Where NUM_TAB = ? ");
		String strQueryPend = new String("Select NUM_TAB, NUM_TAB_DESCRIPTION, NUM_TAB_STATUS_NT, NUM_TAB_STATUS, MAKER, " +
				"RECORD_INDICATOR_NT, RECORD_INDICATOR, VERIFIER, "+dateFormat+"(DATE_LAST_MODIFIED, "+dateFormatStr+") DATE_LAST_MODIFIED, " +
				dateFormat+"(DATE_CREATION, "+dateFormatStr+") DATE_CREATION, INTERNAL_STATUS, READ_ONLY From NUM_TAB_PEND Where NUM_TAB = ? ");
	...
	private List<TabVb> getQueryPopupResultsNtNew(TabVb tabVb) {
		...
		if (tabVb.getSmartSearchOpt() != null && tabVb.getSmartSearchOpt().size() > 0) {
			...
			for (SmartSearchVb data: tabVb.getSmartSearchOpt()){
				...
				case "numTab":
					CommonUtils.addToQuerySearch(" upper(TAppr.NUM_TAB) "+ val, strBufApprove, data.getJoinType());
					CommonUtils.addToQuerySearch(" upper(TPend.NUM_TAB) "+ val, strBufPending, data.getJoinType());
					break;

				case "numTabDescription":
					CommonUtils.addToQuerySearch(" upper(TAppr.NUM_TAB_DESCRIPTION) "+ val, strBufApprove, data.getJoinType());
					CommonUtils.addToQuerySearch(" upper(TPend.NUM_TAB_DESCRIPTION) "+ val, strBufPending, data.getJoinType());
					break;
				...
				case "numTabStatus":
					CommonUtils.addToQuerySearch(" upper(TAppr.NUM_TAB_STATUS) "+ val, strBufApprove, data.getJoinType());
					CommonUtils.addToQuerySearch(" upper(TPend.NUM_TAB_STATUS) "+ val, strBufPending, data.getJoinType());
					break;
		...
		String orderBy=" ORDER BY NUM_TAB";
		return getQueryPopupResults(tabVb,strBufPending, strBufApprove, strWhereNotExists, orderBy, params, getQueryPopupMapper());
	}
	private List<TabVb> getQueryPopupResultsAtNew(TabVb tabVb) {
		setServiceDefaultsNt();
		Vector<Object> params = new Vector<Object>();
		int Ctr = 0;
		List<TabVb> result = null;
		...
		if (tabVb.getSmartSearchOpt() != null && tabVb.getSmartSearchOpt().size() > 0) {
			...
			for (SmartSearchVb data: tabVb.getSmartSearchOpt()){
				...
				switch (data.getObject()) {
				case "alphaTab":
					CommonUtils.addToQuerySearch(" upper(TAppr.ALPHA_TAB) "+ val, strBufApprove, data.getJoinType());
					CommonUtils.addToQuerySearch(" upper(TPend.ALPHA_TAB) "+ val, strBufPending, data.getJoinType());
					break;

				case "alphaTabDescription":
					CommonUtils.addToQuerySearch(" upper(TAppr.ALPHA_TAB_DESCRIPTION) "+ val, strBufApprove, data.getJoinType());
					CommonUtils.addToQuerySearch(" upper(TPend.ALPHA_TAB_DESCRIPTION) "+ val, strBufPending, data.getJoinType());
					break;
				...
				case "alphaTabStatus":
					CommonUtils.addToQuerySearch(" upper(TAppr.ALPHA_TAB_STATUS) "+ val, strBufApprove, data.getJoinType());
					CommonUtils.addToQuerySearch(" upper(TPend.ALPHA_TAB_STATUS) "+ val, strBufPending, data.getJoinType());
					break;
