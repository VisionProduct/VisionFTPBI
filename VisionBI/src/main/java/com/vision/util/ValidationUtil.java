...
//			TimeUnit.SECONDS.sleep(randNum);
	...
	public static boolean isNumberDatatype(int columnType) {
		return (columnType == -6 || columnType == 5 || columnType == 4 || columnType == -5 || columnType == 6 || columnType == 7 || columnType == 8 || columnType == 2
				|| columnType == 3) ? true : false;
	}
	...
	public static boolean isNumericDecimalNew(String pInput) {
		int length = 0;
		if (pInput == null || (length = pInput.length()) == 0) {
			return false;
		}
