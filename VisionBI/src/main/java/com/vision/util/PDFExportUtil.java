...
public class PDFExportUtil {
	...
	static class InvPdfPageEventHelper extends com.itextpdf.text.pdf.PdfPageEventHelper{
		...
		public InvPdfPageEventHelper(String headerText, String footerText, String assetFolderUrl,
				List<PromptIdsVb> prompts,ExportVb exportVb) throws DocumentException, IOException{
			this.headerText = headerText;
			this.footerText = footerText;
			this.assetFolderUrl = assetFolderUrl;
			this.prompts = prompts;
			this.exportVb = exportVb;
			baseFont = BaseFont.createFont(assetFolderUrl+"/CALIBRI.TTF", BaseFont.WINANSI, BaseFont.EMBEDDED);
		}
		@Override
	...
	public  ExceptionCode exportToPdfRAWithGroup(List<ColumnHeadersVb> columnHeaders, List<HashMap<String, String>> dataLst, 
		...
		try {
			...
			try {
							...
							for (int loopCount = 0; loopCount < columnHeadersFinallst.size(); loopCount++) {
								...
								if (type.equalsIgnoreCase(CAP_COL)) {
									...
									if (totalRowFlag || subTotalFlag) {
										ExceptionCode excepCode = writeTotalRowCaptionColDataToTable(reportsVb,loopCount,orgValue,baseFont,table,dataLst,recCount,maxHeaderCol,summaryDataFontSize,cell);
										...
										ExceptionCode excepCode = writeCaptionColDataToTable(reportsVb,loopCount,orgValue,baseFont,table,dataLst,recCount,maxHeaderCol,formatType,dataFontSize,cell);
										if(excepCode.getErrorCode() == Constants.SUCCESSFUL_OPERATION) {
											table = (PdfPTable)excepCode.getResponse();
											cell = (PdfPCell)excepCode.getOtherInfo();	
										}else {
									...
									int unitPrecision = (dataLstMap.get(colHeadersVb.getExtraAttrDecimal())) != null ? 
											Integer.parseInt((String) dataLstMap.get(colHeadersVb.getExtraAttrDecimal())) : Integer.parseInt(colHeadersVb.getDecimalCnt());
									...
									if (ValidationUtil.isValid(orgValue)) {
											...
											if (ValidationUtil.isNumericDecimal(orgValue)) {
												if (!"NR".equalsIgnoreCase(colType) || !"TR".equalsIgnoreCase(colType)) {
													...
													double amount = Double.parseDouble(orgValue.replaceAll(",", ""));
											        DecimalFormat formatter = new DecimalFormat(pattern.toString());
													cellText = formatter.format(amount);
												} else {
													/*Double val = Double.parseDouble(orgValue);
													DecimalFormat formatValue = new DecimalFormat("0.00");
													cellText = formatValue.format(val);*/
													cellText = orgValue;
												}
											} else {
												cellText = orgValue;
									...
									if (totalRowFlag || subTotalFlag) {
										ExceptionCode excepCode = writeTotalRowDataColToTable(reportsVb,loopCount,cellText,baseFont,table,dataLst,recCount,maxHeaderCol,summaryDataFontSize,colHeadersVb,cell);
										if(excepCode.getErrorCode() == Constants.SUCCESSFUL_OPERATION) {
											table = (PdfPTable)excepCode.getResponse();
											cell = (PdfPCell)excepCode.getOtherInfo();	
										}else {
										...
										ExceptionCode excepCode = writeRowDataColToTable(reportsVb,loopCount,baseFont,table,dataLst,recCount,maxHeaderCol,dataFontSize,formatType,cellText,colHeadersVb,cell);
										if(excepCode.getErrorCode() == Constants.SUCCESSFUL_OPERATION) {
											table = (PdfPTable)excepCode.getResponse();
											cell = (PdfPCell)excepCode.getOtherInfo();	
										}else {
				...
				if (totalWidth > pageWidthSize) {
					float newWidth= totalWidth+100;
					float aspectRatio = 0f;
					aspectRatio = pageHeightSize/pageWidthSize;
					float finalPageHeight = aspectRatio*newWidth;
					pageWidthSize = newWidth;
					pageHeightSize = finalPageHeight;
				}
				...
				if (firstTimeFalg) {
					...
					lFile = new File(filePath + ValidationUtil.encode(reportsVb.getReportTitle()) + "_"
							+ currentUserId + ".pdf");
					if (lFile.exists()) {
						lFile.delete();
					}
				...
				float pageWidth = pageWidthSize;
			    float pageHeight =  (pageHeightSize) - (document.topMargin() + document.bottomMargin() + invPdfEvt.headerHeight+50);
			...
			try {
				...
				if (columnHeaders != null) {
					...
					for (ColumnHeadersVb columnHeadersVb : colHeadersCol) {
						...
						int[] sunoidaPinkClr =  getRGB(reportsVb.getApplicationTheme());
						cell.setBackgroundColor(new BaseColor(sunoidaPinkClr[0],sunoidaPinkClr[1],sunoidaPinkClr[2])); // Sunoida Latest Pink
						cell.setUseVariableBorders(true);
						cell.setBorderWidthTop(0.5f);
						cell.setBorderWidthBottom(0.5f);
						cell.setBorderColor(new BaseColor(222,226,230));
						...
						if (columnHeadersVb.getColspan() <= 1) {
							if(columnHeadersVb.getNumericColumnNo() != 99) {
								//float columnWidth = Float.parseFloat(columnHeadersVb.getColumnWidth());
								//float pdfColWdth = widthAfterMargin*columnWidth/100;
								float columnWidth = ColumnText.getWidth(cell.getPhrase());
								//System.out.println("caption : "+columnHeadersVb.getCaption() +" columnWidth : "+columnWidth);
								colSizes.put(columnHeadersVb.getLabelColNum()-1, ColumnText.getWidth(cell.getPhrase()));
								//colSizes.put(columnHeadersVb.getLabelColNum() - 1, pdfColWdth);
								loopCnt++;
							}
					...
					for (HashMap dataLstMap : dataLst) {
						...
						for (int loopCount = 0; loopCount < columnHeadersFinallst.size(); loopCount++) {
							...
							if (type.equalsIgnoreCase(CAP_COL)) {
								...
								table = writeGrowthColDataToTable(reportsVb,loopCount,orgValue,baseFont,table,dataLst,recCount,maxHeaderCol,formatType,dataFontSize,assetFolderUrl);
							} else {
								/*int unitPrecision = ValidationUtil.isValid(colHeadersVb.getDecimalCnt()) ?
										Integer.parseInt(colHeadersVb.getDecimalCnt()) : 2 ;*/
								int unitPrecision = (dataLstMap.get(colHeadersVb.getExtraAttrDecimal())) != null ? 
										Integer.parseInt((String) dataLstMap.get(colHeadersVb.getExtraAttrDecimal())) : Integer.parseInt(colHeadersVb.getDecimalCnt());
								StringBuilder pattern = null;
								// index = ((loopCount+1) - reportsWriterVb.getCaptionLabelColCount());
				...
				File lFile = null;
				lFile = new File(filePath + ValidationUtil.encode(reportsVb.getReportTitle()) + "_" + currentUserId + ".pdf");
				if (lFile.exists()) {
					lFile.delete();
				}
					...
					while (pages > 0) {
						...
						if(rowEnd != table.getRows().size()) {
							...
							if (pages > 0) {
								...
								int additionalRows = maxHeaderRow;
								if(reportsVb.getTotal() != null && !reportsVb.getTotal().isEmpty())
									additionalRows = additionalRows+1;
								if(reportsVb.getTotalRows()+additionalRows >= rowEnd) {
									pdfpRows = table.getRows(rowStart, rowEnd);
									tempTable = createTable(table, pdfpRows, maxHeaderCol, table.getNumberOfColumns(), pages,totalPages, maxHeaderCol);
									tempTable.setTotalWidth(totalWidth);
									tempTable.setWidthPercentage(percentWidth);
									tempTable.setWidths(colsArry);
	...
	private static float drawHeaderAndFootersRA(Document document, ReportsVb reportsVb, 
		...
		return borderTable.calculateHeights(true);
	}
	...
	public PdfPTable  createPromptsPage(Document document,ReportsVb reportsVb) throws MalformedURLException, IOException, DocumentException {
		...
		//reportsVb.setPromptLabel("Vision SBU : 'ALL'!@#Account Officer : 'ALL'!@#Confidence Level : 'ALL'!@#From Date : '1-Jan-2016'!@#To Date : '1-Jan-2021'!@#scalingFactor : 'No Scaling");
		...
		float[] columnWidths = new float[]{10f, 15f};
        tableT.setWidths(columnWidths);

		//tableT.setWidths(new int[]{1, 1});
		...
		insertCell(tableT, reportsVb.getReportTitle(), Element.ALIGN_LEFT, 0,fontLeft,"L");*/
		...
		cell.setColspan(0);
		tableT.addCell(cell);
		//tableT.setTotalWidth(30);
		//insertCell(tableT, "Generated on", Element.ALIGN_LEFT,0, fontLeft,"L");
		//SimpleDateFormat dateFormat = new SimpleDateFormat("MMM dd, yyyy - hh:mm:ss a");
		//insertCell(tableT, dateFormat.format(new Date()), Element.ALIGN_LEFT,0,fontRight,"R");
		//insertCell(tableT, "Generated By", Element.ALIGN_LEFT,0, fontLeft,"L");
		//insertCell(tableT, reportsVb.getMakerName(), Element.ALIGN_LEFT,0, fontRight,"R");
		//insertCell(tableT, "\n", Element.ALIGN_LEFT,0, fontScF);
		//insertCell(tableT, "Powered By Sunoida", Element.ALIGN_CENTER,2, fontRight,"R");
	    return tableT;
...
//			cell.disableBorderSide(2);
	...
	public ExceptionCode dashboardExportToPdf(String screenCapturedImage, ReportsVb dObj, String assetFolderUrl,String fileName,ArrayList<ReportsVb> reportslst) {
		ExceptionCode exceptionCode = new ExceptionCode();
		Document document = null;
		PdfWriter writer = null;
		...
		try {
			...
			if (reportslst != null && reportslst.size() > 0) {
				for(int ctr=0;ctr<reportslst.size();ctr++) {
					ReportsVb reportsVb = (ReportsVb)reportslst.get(ctr);
					...
					groupingFont.setSize(7);
					groupingFont.setStyle(Font.BOLD);
					reportsVb.setReportTitle(reportsDao.getReportTitle(reportsVb.getReportId()));
					...
					if (colHeaderslst != null) {
						...
						for (ColumnHeadersVb columnHeadersVb : colHeadersCol) {
							...
							PdfPCell cell = new PdfPCell(new Phrase(columnHeadersVb.getCaption(), font));
							//cell.setBackgroundColor(new BaseColor(0, 92, 140)); // Default Color blue
							int[] sunoidaPinkClr =  getRGB(dObj.getApplicationTheme());
							cell.setBackgroundColor(new BaseColor(sunoidaPinkClr[0],sunoidaPinkClr[1],sunoidaPinkClr[2]));
	...
	private ExceptionCode writeTotalRowCaptionColDataToTable(ReportsVb reportsVb,int loopCount,String orgValue,BaseFont baseFont,PdfPTable table,List<HashMap<String, String>> dataLst,
			int recCount,int maxHeaderCol,int summaryDataFontSize,PdfPCell cell) {
	...
	private ExceptionCode writeCaptionColDataToTable(ReportsVb reportsVb,int loopCount,String orgValue,BaseFont baseFont,PdfPTable table,List<HashMap<String, String>> dataLst,
			int recCount,int maxHeaderCol,String formatType,int dataFontSize,PdfPCell cell) {
		String cellText = orgValue;
		ExceptionCode exceptionCode = new ExceptionCode();
		try {
			Font font = new Font(baseFont);
	...
	private PdfPTable writeGrowthColDataToTable(ReportsVb reportsVb,int loopCount,String orgValue,BaseFont baseFont,PdfPTable table,List<HashMap<String, String>> dataLst,
			int recCount,int maxHeaderCol,String formatType,int dataFontSize,String assetFolderUrl) {
		PdfPCell cell = null;
		String cellText = "";
		try {
			cellText = "ARROW"; 
	...
	private ExceptionCode writeTotalRowDataColToTable(ReportsVb reportsVb,int loopCount,String orgValue,BaseFont baseFont,PdfPTable table,List<HashMap<String, String>> dataLst,
			int recCount,int maxHeaderCol,int summaryDataFontSize,ColumnHeadersVb colHeadersVb,PdfPCell cell) {
		String cellText = orgValue;
		ExceptionCode exceptionCode = new ExceptionCode();
		try {
			Font font = new Font(baseFont);
			...
			if("Y".equalsIgnoreCase(colHeadersVb.getColorDiff())) {
				if(cellText.substring(0, 1).contains("-"))
					font.setColor(new BaseColor(250,42,42));
				else
					font.setColor(new BaseColor(145,207,77));
			}
	...
	private ExceptionCode writeRowDataColToTable(ReportsVb reportsVb,int loopCount,BaseFont baseFont,PdfPTable table,List<HashMap<String, String>> dataLst,
			int recCount,int maxHeaderCol,int dataFontSize,String formatType,String cellText,ColumnHeadersVb colHeadersVb,PdfPCell cell) {
		ExceptionCode exceptionCode = new ExceptionCode();
		try {
			Font font = new Font(baseFont);
			font.setSize(dataFontSize);
			cell = new PdfPCell(new Phrase(cellText, font));
			if("FT".equalsIgnoreCase(formatType) || "S".equalsIgnoreCase(formatType) || "ST".equalsIgnoreCase(formatType) || "Y".equalsIgnoreCase(colHeadersVb.getColorDiff())) {
				font.setStyle(Font.BOLD);
				if("S".equalsIgnoreCase(formatType) || "ST".equalsIgnoreCase(formatType)) {
					font.setSize(summaryDataFontSize);
					cell.setBackgroundColor(new BaseColor(242,244,242));//light grey
	...
	public ExceptionCode dashboardExportNewforPdf(ExportVb dObj,String assetFolderUrl,int currentUserId) {
		ExceptionCode exceptionCode =  new ExceptionCode();
		Document document = null;
		PdfWriter writer = null;
		Rectangle rectangle = null;
		...
		try {
			...
			if(ValidationUtil.isValid(dObj.getPdfGrwthWidth()))
				grwthWidth = Float.parseFloat(dObj.getPdfGrwthWidth());
			
			float newWidth= pageWidthSize+grwthWidth;
			float aspectRatio = 0f;
			aspectRatio = pageHeightSize/pageWidthSize;
			float finalPageHeight = aspectRatio*newWidth;
			pageWidthSize = newWidth;
			...
			float widthAfterMargin = pageWidthSize - 40;
			ReportsVb vObject =  new ReportsVb();
			vObject.setApplicationTheme(dObj.getApplicationTheme());
			vObject.setReportTitle(dObj.getDashboardName());
			vObject.setMakerName(dObj.getMakerName());
			vObject.setPromptLabel(dObj.getPromptLabel());
			String filePath = System.getProperty("java.io.tmpdir");
			if (!ValidationUtil.isValid(filePath)) {
				filePath = System.getenv("TMP");
			}
			...
			File lFile = null;
			lFile = new File(filePath + dObj.getDashboardName() + "_" + currentUserId + ".pdf");
			if (lFile.exists()) {
				lFile.delete();
			}
			BaseFont baseFont = BaseFont.createFont(assetFolderUrl + "/CALIBRI.TTF", BaseFont.WINANSI,BaseFont.EMBEDDED);
			...
			FileOutputStream fips = new FileOutputStream(lFile);
			writer = PdfWriter.getInstance(document, fips);
			InvPdfPageEventHelper invPdfEvt = new InvPdfPageEventHelper(null, vObject.getMakerName(), assetFolderUrl,
					new ArrayList(), vObject);
			writer.setPageEvent(invPdfEvt);
			document.open();
			PdfPTable tableT = null;
			tableT = createPromptsPage(document, vObject);
			document.add(tableT);
			// Add screenshot in PDF
			for (int tab = 0; tab < dObj.getExportTabDetailsLst().size(); tab++) {
				ExportVb exportVb = new ExportVb();
				exportVb = (ExportVb) dObj.getExportTabDetailsLst().get(tab);
				document.newPage();
				table = new PdfPTable(1);
				PdfPCell cellImage = new PdfPCell();
				String screenCapturedPath = commonDao.findVisionVariableValue("PRD_EXPORT_PATH");
				String capturedImage =screenCapturedPath+exportVb.getExportFileName()+".png";
				
				String base64ImageString = dObj.getExportImage().replace("data:image/png;base64,", "");
				...
				Image image = Image.getInstance(capturedImage);
				image.setScaleToFitLineWhenOverflow(true);
				table.setWidthPercentage(100);
				float imgWidth = document.getPageSize().getWidth() - document.leftMargin() - document.rightMargin();
				float imgHeight = document.getPageSize().getHeight() - document.topMargin() - document.bottomMargin();
				...
				dashboardTabVb.setDashboardId(dObj.getDashboardId());
				dashboardTabVb.setTabId(exportVb.getTabId());
				List<DashboardTilesVb> tileslst = dashboardsDao.getDashboardTileDetailsPdf(dashboardTabVb, true,"'G','T'");//G T
				/*if(dObj.getExportFlag()) {
					DashboardTilesVb dashboardTilesVb = new DashboardTilesVb();
					dashboardTilesVb.setDashboardId(dObj.getDashboardId());
					dashboardTilesVb.setTabId(dObj.getTabId());
					dashboardTilesVb.setTileSequence(Integer.parseInt(dObj.getTileSequence()));
					tileslst.add(dashboardTilesVb);
				}
				*/
				if (dObj.getExportFlag()) {
					String[] tileSeqArr = String.valueOf(dObj.getTileSequence()).split("!@#");
					for (int i = 0; i < tileSeqArr.length; i++) {
						DashboardTilesVb dashboardTileVb = new DashboardTilesVb();
						dashboardTileVb.setDashboardId(dObj.getDashboardId());
						dashboardTileVb.setTabId(dObj.getTabId());
						dashboardTileVb.setTileSequence(Integer.parseInt(tileSeqArr[i]));
						tileslst.add(dashboardTileVb);
					}
				}
				//dashboardTilesVb.setTileType(dObj.getTileType());
				List<DashboardTilesVb> tileLstNew = new ArrayList<>();
				if(tileslst != null && tileslst.size() > 0) {
					for(DashboardTilesVb tileVb : tileslst) {
						if("G".equalsIgnoreCase(tileVb.getTileType())) {
							tileLstNew.add(tileVb);
							continue;
						}
						List singletonChildLst = dashboardsDao.getTileDrillDownDetailsforExport(tileVb.getDashboardId(), tileVb.getTabId(), 
								tileVb.getTileSequence(), tileVb.getSubSequence(), false);
						if(singletonChildLst != null && singletonChildLst.size() > 0) {
							DashboardTilesVb tilesVb = (DashboardTilesVb)singletonChildLst.get(0);
							tilesVb.setIsDrillDown(true);
							tileLstNew.add(tilesVb);
						}
					}
				}
				if (tileLstNew != null && tileLstNew.size() > 0 ) {
					
					//if (gridCount > 0) {
						
						for (DashboardTilesVb tilesVb : tileLstNew) {
							document.newPage();
							PdfPTable groupingTable = new PdfPTable(1);
							groupingTable.setWidthPercentage(100);
							PdfPCell cellG;
							Font groupingFont = new Font(baseFont);
							groupingFont.setSize(9);
							groupingFont.setStyle(Font.BOLD);
							String tileCaption = "";
							tileCaption = tilesVb.getTileCaption();
							if(tilesVb.getIsDrillDown())
								tileCaption = tileCaption+" (Drill down)";
							Phrase ph = new Phrase(tileCaption, groupingFont);
							Paragraph p = new Paragraph(ph);
							cellG = new PdfPCell(p);
							cellG.setHorizontalAlignment(Element.ALIGN_LEFT);
							cellG.setBorder(0);
							...
							reportsVb.setApplicationTheme(dObj.getApplicationTheme());
							ExceptionCode exceptionCodeTiles = new ExceptionCode();
							tilesVb.setPromptValue1(exportVb.getPromptValue1());
							tilesVb.setPromptValue2(exportVb.getPromptValue2());
							tilesVb.setPromptValue3(exportVb.getPromptValue3());
							tilesVb.setPromptValue4(exportVb.getPromptValue4());
							tilesVb.setPromptValue5(exportVb.getPromptValue5());
							tilesVb.setPromptValue6(exportVb.getPromptValue6());
							tilesVb.setPromptValue7(exportVb.getPromptValue7());
							tilesVb.setPromptValue8(exportVb.getPromptValue8());
							tilesVb.setPromptValue9(exportVb.getPromptValue9());
							tilesVb.setPromptValue10(exportVb.getPromptValue10());
							tilesVb.setApplicationTheme(dObj.getApplicationTheme());
							exceptionCodeTiles = dashboardsDao.getGridData(tilesVb);
							if (exceptionCodeTiles.getErrorCode() == Constants.SUCCESSFUL_OPERATION
									&& exceptionCodeTiles.getResponse() != null) {
								gridDatalst = (List) exceptionCodeTiles.getResponse();
							}
							...
							if (gridDatalst.size() > 0 && gridDatalst != null) {
								columnHeaders = (ArrayList<ColumnHeadersVb>) gridDatalst.get(0);
								dataLst = ((List) gridDatalst.get(1));
							}
							int colNum = 0;
							int rowNum = 1;
							//int loopCnt = 0;
							...
							if (columnHeaders != null && !columnHeaders.isEmpty()) {
								...
								List<ColumnHeadersVb> colHeadersCol = columnHeaders.stream().sorted(Comparator.comparingInt(ColumnHeadersVb::getLabelRowNum))
								...
								if (columnHeaders != null) {
									...
									for (ColumnHeadersVb columnHeadersVb : colHeadersCol) {
										...
										PdfPCell cell = new PdfPCell(new Phrase(columnHeadersVb.getCaption(), font));
										// cell.setBackgroundColor(new BaseColor(0, 92, 140)); // Default Color blue
										int[] sunoidaPinkClr = getRGB(tilesVb.getApplicationTheme());
										cell.setBackgroundColor(new BaseColor(sunoidaPinkClr[0], sunoidaPinkClr[1], sunoidaPinkClr[2])); // Sunoida Latest Pink
										cell.setUseVariableBorders(true);
										cell.setBorderWidthTop(0.5f);
										cell.setBorderWidthBottom(0.5f);
								...
								if (dataLst != null && !dataLst.isEmpty()) {
									do {
										for (HashMap dataLstMap : dataLst) {
											...
											for (int loopCount = 0; loopCount < columnHeadersFinallst.size(); loopCount++) {
												...
												if (type.equalsIgnoreCase(CAP_COL)) {
													...
													if (dataLstMap.containsKey(colHeadersVb.getDbColumnName())) {
														orgValue = ((dataLstMap.get(colHeadersVb.getDbColumnName())) != null
																|| dataLstMap.get(colHeadersVb.getDbColumnName()) == "")? dataLstMap.get(colHeadersVb.getDbColumnName()).toString(): "";
													}

													cellText = orgValue;
													if (totalRowFlag) {
														ExceptionCode excepCode = writeTotalRowCaptionColDataToTable(reportsVb, loopCount, orgValue, baseFont, table,
																dataLst, recCount, maxHeaderCol, summaryDataFontSize,
																cell);
														if (excepCode.getErrorCode() == Constants.SUCCESSFUL_OPERATION) {
															table = (PdfPTable) excepCode.getResponse();
															cell = (PdfPCell) excepCode.getOtherInfo();
														...
														ExceptionCode excepCode = writeCaptionColDataToTable(reportsVb,loopCount, orgValue, baseFont, table, dataLst, recCount,
																maxHeaderCol, formatType, dataFontSize, cell);
														if (excepCode.getErrorCode() == Constants.SUCCESSFUL_OPERATION) {
															table = (PdfPTable) excepCode.getResponse();
															cell = (PdfPCell) excepCode.getOtherInfo();
														} else {
													...
													table = writeGrowthColDataToTable(reportsVb, loopCount, orgValue,baseFont, table, dataLst, recCount, maxHeaderCol,
															formatType, dataFontSize, assetFolderUrl);
												} else {
													// index = ((loopCount+1) -
													// reportsWriterVb.getCaptionLabelColCount());
													if (dataLstMap.containsKey(colHeadersVb.getDbColumnName()))
														orgValue = ((dataLstMap.get(colHeadersVb.getDbColumnName())) != null
																|| dataLstMap.get(colHeadersVb.getDbColumnName()) == "")? dataLstMap.get(colHeadersVb.getDbColumnName()).toString(): "";
													else
														orgValue = "";
													// String orgValue =
													// dataLstMap.containsKey(colHeadersVb.getDbColumnName())?
													// dataLstMap.get(colHeadersVb.getDbColumnName()).toString(): "";
													if (ValidationUtil.isValid(orgValue)) {
														if ("I".equalsIgnoreCase(colType) || "S".equalsIgnoreCase(colType)) {
															if (ValidationUtil.isNumericDecimal(orgValue)) {
													...
													if (ValidationUtil.isValid(cellText)
															&& "-0.00".equalsIgnoreCase(cellText)) {
														cellText = "0.00";
													}
													if (totalRowFlag) {
														ExceptionCode excepCode = writeTotalRowDataColToTable(reportsVb,loopCount, cellText, baseFont, table, dataLst, recCount,
																maxHeaderCol, summaryDataFontSize,colHeadersVb, cell);
														if (excepCode.getErrorCode() == Constants.SUCCESSFUL_OPERATION) {
															table = (PdfPTable) excepCode.getResponse();
															cell = (PdfPCell) excepCode.getOtherInfo();
														} else {
														...
														ExceptionCode excepCode = writeRowDataColToTable(reportsVb,loopCount, baseFont, table, dataLst, recCount,
																maxHeaderCol, dataFontSize, formatType, cellText,colHeadersVb, cell);
														if (excepCode.getErrorCode() == Constants.SUCCESSFUL_OPERATION) {
															table = (PdfPTable) excepCode.getResponse();
															cell = (PdfPCell) excepCode.getOtherInfo();
														} else {
										...
										dataLst = new ArrayList();
										if (!totalRowFlag) {
											exceptionCode = reportsWb.getResultData(reportsVb);
											if (ValidationUtil.isValid(exceptionCode.getResponse())) {
												ReportsVb resultVb = (ReportsVb) exceptionCode.getResponse();
												dataLst = resultVb.getGridDataSet();
												if (dataLst == null || dataLst.isEmpty()) {
								...
								float pageWidth = "L".equalsIgnoreCase(reportsVb.getReportOrientation())? PageSize.A4.getHeight(): PageSize.A4.getWidth();
								float pageHeight = ("L".equalsIgnoreCase(reportsVb.getReportOrientation())? PageSize.A4.getWidth()
										: PageSize.A4.getHeight())- (document.topMargin() + document.bottomMargin() + invPdfEvt.headerHeight+ 50);
								// float pageWidth = reportsVb.getPdfWidth();
								pageWidth = (pageWidth - (document.leftMargin() + document.rightMargin()));
								/* if (totalWidth < pageWidth) { */
								float percentWidth = (float) ((totalWidth * 100) / (pageWidth - 200));
								...
								while (pages > 0) {
									...
									if (rowEnd != table.getRows().size()) {
										...
										pages--;
										/*
										 * if (pages - 1 != 0) rowEnd = rowEnd - table.getHeaderRows();
										 */
										if (pages > 0)
											document.newPage();
									} else {
			...
			writer.close();
			exceptionCode.setResponse(filePath);
			exceptionCode.setOtherInfo(dObj.getDashboardName() + "_" + currentUserId);
			exceptionCode.setErrorCode(Constants.SUCCESSFUL_OPERATION);
		}catch(Exception e) {
			exceptionCode.setErrorCode(Constants.ERRONEOUS_OPERATION);
			exceptionCode.setErrorMsg(e.getMessage());
	...
	public  ExceptionCode exportToPdfDashboard(List<ColumnHeadersVb> columnHeaders, List<HashMap<String, String>> dataLst, ReportsVb reportsVb, 
			String assetFolderUrl,List<String> columnTypes,int currentUserId,List<HashMap<String, String>> totalLst,ArrayList<ColumnHeadersVb> columnHeadersFinallst){
		...
		try {
			...
			if("P".equalsIgnoreCase(reportsVb.getReportOrientation())) {
				...
				if(columnHeadersFinallst.size() > 15) {
					...
					colheaderFontSize = (int)9;
					dataFontSize = (int)9;
					summaryDataFontSize = (int)9;
